...
# This file is the single implementation shared by every euroscope cask in the
# tap; the @@...@@ placeholders are substituted per cask at install time. Edit
# it here, never inside a cask. The casks are mutually exclusive
# (conflicts_with), so one prefix, one app bundle and one `euroscope` command
# are all that ever exist at a time.
ES_VERSION="@@VERSION@@"
ES_TOKEN="@@TOKEN@@"
